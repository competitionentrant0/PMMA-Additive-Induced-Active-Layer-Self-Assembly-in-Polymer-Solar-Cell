...
int main() {
	...
	for (double i=156.0; i<1344.0; i++) { //test from edge to edge between columns, incrementing by 1 nanometer at a time
		for (double j=1.0; j<90.0; j++) { //test incident angles from 1 to 90 degrees
			cout << i-156.0 << " nanometers from left edge, " << j << " degrees" << endl;
			double slope = tan (j * PI / 180.0); //slope of incident light
			double xCoord = i;
			double yCoord = 158.0;
			...
			while (true) {
				...
				or (abs(1500.0 - pow(yCoord - 79.0, 2)/40.0 - xCoord) <= 0.001)) { //extend ray incrementally till close enough to "hit" parabola
					xIntersect = xCoord; 
					yIntersect = yCoord;
					if (xIntersect <= 0.001) { //at the middle point of left parabola
					...
					else if (xIntersect >= 1499.999) { //at the middle point of right parabola
						parabolaTangent[0] = 1499.998; parabolaTangent[1] = 1499.998;
						parabolaTangent[2] = sqrt(40.0 * (1500.0 - 1499.998)) + 79.0;
						parabolaTangent[3] = - sqrt(40.0 * (1500.0 - 1499.998)) + 79.0;
					}
					else { 
						if (xCoord < 500) { //if intersect at left parabola
							...
							parabolaTangent[0] = xIntersect - 0.001; parabolaTangent[1] = xIntersect + 0.001;
							parabolaTangent[2] = sqrt(40.0 * (1500.0 - parabolaTangent[0])) + 79.0;
							parabolaTangent[3] = sqrt(40.0 * (1500.0 - parabolaTangent[1])) + 79.0; 
						}                                                                                                                             
					}
	...
	for(int a=156.0; a<1344.0; a++) {
		for (int b=1; b<90; b++) {
			...
			while (true){
				...
				if (xCoord <= 78 or xCoord >= 1422) { //limit to the same scope tested above
					double dist3 = distance(a, xCoord, 158.0, yCoord);
